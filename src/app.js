...
if (allowedOrigins.length === 0) {
  logger.error('[CORS] No CLIENT_URL set — all cross-origin requests will be rejected.');
}
...
const csrfGuard = (req, res, next) => {
  ...
  if (!allowed || !origin.startsWith(allowed)) {
    logger.warn(`[CSRF] Blocked: origin="${origin}" method=${req.method} path=${req.path}`);
...
if (process.env.NODE_ENV !== 'production') {
  ...
  app.use(
    morgan('combined', {
      ...
      skip: (req) => req.url === '/health' || req.url.startsWith('/api/products') && req.method === 'GET',
    }),
  );
}
